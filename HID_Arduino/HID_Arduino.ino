...
void loop()
{
    ...
    // Process ALL incoming serial commands from sunone first
    ...
    // AIMBOT active: gunakan aimbot movement saja (ignore mouse fisik)
    if (aim_delta[0] != 0 || aim_delta[1] != 0 || aim_delta[2] != 0)
    {
        Mouse.move(aim_delta[0], aim_delta[1], aim_delta[2]);
    }
    // AIMBOT idle: passthrough mouse fisik normal
    else
    {
        Mouse.move(delta[0], delta[1], delta[2]);
    }

    // Reset aim delta for next loop
    memset(aim_delta, 0, sizeof(aim_delta));
}
...
void ExecuteMouseMoveCommand(const String& command)
{
    ...
    aim_delta[0] += constrain(x, -127, 127);
    aim_delta[1] += constrain(y, -127, 127);
}
